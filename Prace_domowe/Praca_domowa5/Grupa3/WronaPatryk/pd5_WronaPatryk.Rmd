...
```{r}
plot(x = df$V1, y = df$V2, xlab = "V1", ylab = "V2",
...
plot(x = df$V1, y = df$V2, col = spectral, xlab = "V1", ylab = "V2",
...
plot(x = df$V1, y = df$V2, col = cutree(hclust.labels ,k = 2), xlab = "V1", ylab = "V2",
...
```{r}
ggplot(data = data, aes(x = V1, y = V2)) + geom_point(aes(colour = factor(cluster))) + facet_wrap(~ method) + theme(legend.position = "none")
```
